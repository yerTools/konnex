...
RUN CGO_ENABLED=0 GOOS=$TARGETOS GOARCH=$TARGETARCH go build -ldflags="-s -w" -o konnex ./main.go
...
LABEL org.opencontainers.image.source=https://github.com/yerTools/konnex
...
COPY --from=backend_builder /app/konnex /app/konnex

# Make the binary executable
RUN chmod +x /app/konnex
...
CMD ["/app/konnex", "serve", "--http=0.0.0.0:8161"]

# Build this image with:
# docker build -t konnex .
#
# Run this image with:
# docker run -p 8161:8161 konnex
#
...
# docker run -p 8161:8161 -e APP_ENV=production -e DEBUG=false konnex
#
# For multi-architecture builds use:
# docker buildx build --platform linux/amd64,linux/arm64 -t konnex . --push
